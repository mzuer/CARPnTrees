#! /usr/bin/Rscript                         
# shebang may need to be changed according to your R path
...
source("setParam.R")   # load some parameters information

source("CARPfcts.R")   # load functions used in this script

if(.Platform$OS.type == "unix" & wd !=""){
# if (Sys.info()["sysname"] != "Windows") 
  if(substr(wd, nchar(wd), nchar(wd))!="/"){  # ensure that wd ends with "/"
    wd <- paste0(wd, "/")                     # (needed for loading and saving files)
  }                                           # (by default the pwd, can be changed in setParam.R)
  setwd(wd)
}


# ************************ Workflow ********************************
# str data -> matrix -> distance matrix --NJ--> tree               *
# seq data --comparison--> distance matrix --NJ--> tree            *
# All haplogroups tree -> Haplogroups of interest tree             *
# Place samplesID on the tree                                      *
# The ones at the nodes -> moved to tips                           *
# Where multiple samples on 1 tip -> recent polymorphism tree      * 
# ******************************************************************

# If the program is not run from the command line:
# - set parameters in "setParam.R"
# # - comment the next 6 lines
loadPck("optparse")    
analysis <- getOpt()[1]
polyFile <- getOpt()[2]
haploFile <- getOpt()[3]
verb <- getOpt()[4]                    # verbose or not ? (logical)
allHaploTreeName <- getOpt()[5]        # which file to take that contains whole haplogroups tree
...
saveTreeTxt(polyTree, "Recent_R", verb)
# uncomment the following line if you want to save tree as ".png"
#saveTreePlot(polyTree, "recent_poly_without_labR", verb)
if(verb){
...
saveTreeTxt(haploTree, aux="Ancient_R", verb)   # aux -> append "HG" (for HaploGroup) at the end of file name
# uncomment the following line if you want to save tree as ".png"
#saveTreePlot(haploTree, aux="HG_R", verb)   # aux -> append "HG" (for HaploGroup) at the end of file name
...
saveTreeTxt(haploTreeID, aux="AncientWithID_R", verb)
haploTreeID$tip.label <- mixedFontLabel(tipsHG, tipsID, bold = 1, italic = 2, sep=" ")
# uncomment the following line if you want to save tree as ".png"
#saveTreePlot(haploTreeID, aux="HGwithID_R", verb)
...
saveTreeTxt(tempTree, aux="ConsensusTree_R", verb)

haploTree$node.label <- mixedFontLabel(haploTree$node.label, bold = 1)
# uncomment the following line if you want to save tree as ".png"
#saveTreePlot(haploTree, aux="HGandPoly_with_lab_R", verb)

                  # pdf(paste0(wd, "consensusTree.pdf"))
                  # plot(haploTree)
                  # nodelabels(haploTree$node.label, col="red", adj=0, frame="n")
                  # foo <- dev.off()  # so it doesn't print "null device"
...
# uncomment the following line if you want to save tree as ".png"
#saveTreePlot(haploTree, aux="HGandPoly_without_lab_R", verb)              

haploTree$node.label <- sapply(as.character(haploTree$node.label),function(x) "")
saveTreeTxt(haploTree, aux="ConsensusTreeWithoutNodLab_R", verb)
